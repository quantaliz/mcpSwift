...
struct ResourceTests {
    @Test("MCPResource initialization with valid parameters")
    ...
    @Test("MCPResource encoding and decoding")
    func testResourceEncodingDecoding() throws {
        ...
        let decoded = try decoder.decode(MCPResource.self, from: data)
    ...
    @Test("MCPResource.Content text initialization and encoding")
    func testResourceContentTextEncoding() throws {
        let content = MCPResource.Content.text(
            "Hello, world!", uri: "file://test.txt", mimeType: "text/plain")
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()

        let data = try encoder.encode(content)
        let decoded = try decoder.decode(MCPResource.Content.self, from: data)
    ...
    @Test("MCPResource.Content binary initialization and encoding")
    func testResourceContentBinaryEncoding() throws {
        let binaryData = "Test binary data".data(using: .utf8)!
        let content = MCPResource.Content.binary(
    ...
    func testListResourcesResult() throws {
        let resources = [
            MCPResource(name: "resource1", uri: "file://test1.txt"),
            MCPResource(name: "resource2", uri: "file://test2.txt")
        ]
    ...
    func testReadResourceResult() throws {
        let contents = [
            MCPResource.Content.text("Content 1", uri: "file://test1.txt"),
            MCPResource.Content.text("Content 2", uri: "file://test2.txt")
        ]
